...
*/
 ...
 $other = mysql_connect('localhost','root','root');
...
*/
 ...
 $conn3 = mysql_connect('localhost','root','root',true);
 ...
 mysql默认占用的3306,因为，我们在mysql_connert中并没有指定端口
 而PHP会用3306来进行连接
 
 如果，你的mysql服务器端口偏是3307，如何指定
 答：mysql_connect('localhost:3307','root','root',true);

 在php中，只能用mysql_connect函数才能连上mysql服务器？
 答：还可以用mysqli和PDO
 ...
 答：可以用oci_*系列函数 和 mysql_*系列函数

问：太麻烦了，要是换服务器了，岂不是我的程序要重写？
答：不用怕，PDO则是一个统一的数据库接口。
对于程序员来说，屏蔽了数据库之间的不一致。
无论什么数据库，用PDO的写法都一致。

另：PDO也是面向对象的用法。

因此：我们将mysql_*系列面向过程的函数。
后面会讲通用的PDO
