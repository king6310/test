var v = new Vue({
        ...
        sayHi: function() {
            if(this.keywordtxt) {
                this.checkedItems.push({
                    type: 7,
                    content: this.keywordtxt
                })
            }
        },
        defaultVal(v) {
            this.checkedItems.push(v[0])
        },
        deleteItem(index) {
        ...
        checkedItems(val, oldVal) {
            ...
            val.filter(function(task){
                if(task.type == 1) {
                    ...
                    $this.keyword.push(task.content)
                }
            }).length;

            setTimeout(()=>{
                console.log($this.QuickShipVal)
            },1000)
    ...
    mounted () {
        console.log($("#defaultval").val())
        
        this.defaultVal(this.QuickShip)
    }
});